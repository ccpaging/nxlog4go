...
var glog = l4g.NewLogger(l4g.DEBUG).Set("prefix", "example").Set("format", "[%T %D %Z] [%L] (%P:%S) %M")
...
func main() {
	...
	log1 := l4g.NewLogger(l4g.DEBUG).Set("prefix", "example").Set("format", "%P[%T %D %Z] [%L] (%S:%N) %M")
